<template>
	<view class="pretty-number-item">
		<view class="title" @click="clickTitle">
			<label v-html="item.title"></label>
			<uv-image :src="img"  width="25px" height="25px" mode="widthFix"></uv-image>
		</view>
		<view class="body">
			<view class="chd desc">{{item.desc}}</view>
			<uv-button v-if="showBtn" type="error" size="mini" class="btn chd" text="办理号码"></uv-button>
		</view>
		<view class="foot">
			<label>上海移动</label>
			<text>靓</text>
			<uv-text type="error" mode="price" :text="item.price" style="line-height: .8rem;margin-left: .8rem;"></uv-text>
			<uv-icon :name="item.isFav?'heart-fill':'heart'" color="red" size="20" @click="clickFavorite"></uv-icon>
		</view>
	</view>
</template>

<script>
	import store from '@/store/index.js';
	
	const img='data:image/png;base64,';
	
	export default {
		props:['item','showBtn'],
		data() {
			return {
				img
			};
		},
		methods:{
			clickTitle(){
				uni.navigateTo({
					url:'/pages/detail/index?id='+this.item.id
				})
			},
			clickFavorite(){
				this.item.isFav = !this.item.isFav;
				store.dispatch('addFavorite', this.item);
			}
		}
	}
</script>

<style lang="scss" scoped>
.pretty-number-item{
	display: flex;
	flex-direction: column;
	align-items: flex-start;
	padding: .5rem;
	border:1px solid #e5e5e5;
	border-radius: .3rem;
	background-color: #fff;
	.title{
		display: flex;
		justify-content: space-between;
		width: 100%;
		font-size: 1rem;
		font-weight: bold;
	}
	.body{
		display: flex;
		justify-content: space-between;
		width: 100%;
		.chd{
			flex: 1 1 auto;
		}
		.desc{
			width: 100%;
			font-size: .1rem;
			text-align: left;
		}
		.btn{
			width: 100px;
		}
	}
	
	.foot{
		display: flex;
		justify-content: space-between;
		width: 100%;
		margin-top: .5rem;
		
		label{
			display: block;
			padding: 2px;
			height: .8rem;
			line-height: .8rem;
			font-size: 10px;
			background-color: #1da7fd;
		}
		text{
			display: block;
			height: .8rem;
			line-height: .8rem;
			margin-left: 5px;
			padding: 2px;
			background-color: #f0430d;
			box-shadow: 0 0.2rem 0.4rem rgb(0 0 0 / 10%);
			border-radius: 2px;
			font-size: 10px;
			color: white;
		}
	}
}

</style>