...
import nextConfig from './next-paging-config'
import nextLocalConfig from '../config/index'
import c from './next-paging-constant'

const storageKey = 'NEXT-PAGING-REFRESHER-TIME-STORAGE-KEY';
let config = null;
const timeoutMap = {};
...
// #ifdef VUE2
try {
	const contextKeys = require.context('@/uni_modules/next-paging', false, /\next-paging-config$/).keys();
	if (contextKeys.length) {
		const suffix = '.js';
		config = require('@/uni_modules/next-paging/next-paging-config' + suffix);
	}
} catch (e) {}
// #endif
...
function gc(key, defaultValue) {
	if (!config) {
		if (nextLocalConfig && Object.keys(nextLocalConfig).length) {
			config = nextLocalConfig;
		} else {
			const tempConfig = nextConfig.getConfig();
