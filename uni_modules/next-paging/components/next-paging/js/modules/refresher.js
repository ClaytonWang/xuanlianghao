...
export default {
	props: {
		...
		//是否使用自定义的下拉刷新，默认为是，即使用next-paging的下拉刷新。设置为false即代表使用uni scroll-view自带的下拉刷新，h5、App、微信小程序以外的平台不支持uni scroll-view自带的下拉刷新
		useCustomRefresher: {
			type: Boolean,
		...
		//如果需要区别不同页面的最后更新时间，请为不同页面的next-paging的`refresher-update-time-key`设置不同的字符串
