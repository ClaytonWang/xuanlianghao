...
export default {
	props: {
		//使用页面滚动，默认为否，当设置为是时则使用页面的滚动而非此组件内部的scroll-view的滚动，使用页面滚动时next-paging无需设置确定的高度且对于长列表展示性能更高，但配置会略微繁琐
		usePageScroll: {
			type: Boolean,
	...
	methods: {
		...
		//当使用页面滚动并且自定义下拉刷新时，请在页面的onPageScroll中调用此方法，告知next-paging当前的pageScrollTop，否则会导致在任意位置都可以下拉刷新
		...
		//更新next-paging内置scroll-view的scrollTop
			...
			try {
				...
				this.$nextTick(() => {
					...
					if (dataType === '[object Number]') {
						const els = refs[`next-paging-${sel}`];
						el = els ? els[0] : null;
					...
					if (el) {
						...
						u.consoleErr('在nvue中滚动到指定位置，cell必须设置 :ref="`next-paging-${index}`"');
					}
					return;
					// #endif
		...
		//检测next-paging是否要全屏覆盖(当使用页面滚动并且不满全屏时，默认next-paging需要铺满全屏，避免数据过少时内部的empty-view无法正确展示)
