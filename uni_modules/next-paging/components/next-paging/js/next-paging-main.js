...
import zStatic from './next-paging-static'
import c from './next-paging-constant'
import u from './next-paging-utils'

import nextPagingRefresh from '../components/next-paging-refresh'
import nextPagingLoadMore from '../components/next-paging-load-more'
import nextPublicVirtualCell from '../components/next-public-virtual-cell'
import nextPagingEmptyView from '../../next-paging-empty-view/next-paging-empty-view'
...
import Enum from './next-paging-enum'
...
export default {
	...
	props: {
		...
		//设置next-paging的style，部分平台(如微信小程序)无法直接修改组件的style，可使用此属性代替
		pagingStyle: {
			type: Object,
			default: function() {
		...
		//next-paging的高度，优先级低于pagingStyle中设置的height；传字符串，如100px、100rpx、100%
		height: {
			type: String,
			default: u.gc('height', '')
		},
		//next-paging的宽度，优先级低于pagingStyle中设置的width；传字符串，如100px、100rpx、100%
		width: {
			type: String,
			default: u.gc('width', '')
		},
		//next-paging的背景色，优先级低于pagingStyle中设置的background。传字符串，如"#ffffff"
		bgColor: {
			type: String,
			default: u.gc('bgColor', '')
		},
		//设置next-paging的容器(插槽的父view)的style
		pagingContentStyle: {
			type: Object,
			default: function() {
		...
		//next-paging是否自动高度，若自动高度则会自动铺满屏幕
		autoHeight: {
			type: Boolean,
			default: u.gc('autoHeight', false)
		},
		//next-paging是否自动高度时，附加的高度，注意添加单位px或rpx，若需要减少高度，则传负数
		autoHeightAddition: {
			type: [Number, String],
		...
		//next-paging是否使用fixed布局，若使用fixed布局，则next-paging的父view无需固定高度，next-paging高度默认为100%，默认为是(当使用内置scroll-view滚动时有效)
		fixed: {
			type: Boolean,
		...
		//next-paging内容容器父view的z-index，默认为1
		superContentZIndex: {
			type: Number,
			default: u.gc('superContentZIndex', 1)
		},
		//next-paging内容容器部分的z-index，默认为10
		contentZIndex: {
			type: Number,
	...
	methods: {
		//当前版本号
		getVersion() {
			return `next-paging v${c.version}`;
		},
