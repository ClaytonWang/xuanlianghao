...
<template>
	<view class="zp-swiper-item-container">
		<next-paging ref="paging" :fixed="false" 
...
<script>
	import zPaging from '../next-paging/next-paging'
	export default {
		name: "next-paging-swiper-item",
		components: { zPaging },
		data() {
			return {
				firstLoaded: false
		...
		props: {
			...
			//是否在next-paging内部循环渲染列表(内置列表)，默认为否。若use-virtual-list为true，则此项恒为true
			useInnerList: {
				type: Boolean,
				default: false
			},
